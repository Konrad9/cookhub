// passes the right parameters to paginationHandler to display the recipes and
// bind the buttons to events and tells them what to do
function start(csrf_token, authenticated, NewestRecipePages, PopularRecipePages, recipe_pagination, RecipesPerPage) {   
            ...
            if (nextPageNumber >= NewestRecipePages) { // if we are on the only page or there are none, there is no next
            ...
            if (nextPageNumber >= PopularRecipePages) { // if we are on the only page or there are none, there is no next
